...
namespace Elearning.Web.Controllers
{
    public class DMChucDanhController : BaseController
    {
        ...
        public DMChucDanhController(IDMRepository DMRepository, ILogger<DMTinhController> logger, IMedAuthenticateConnect authenticateConnect, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> ExportExcel(DMSearchRequest @base)
        {
            ...
            var workbook = ExcelService.ExportExcel(mappingHeader, data.Data.Cast<dynamic>().ToList(), "Danh sách chức danh");
            var memoryStream = new MemoryStream();
            workbook.Save(memoryStream, new OoxmlSaveOptions(SaveFormat.Xlsx));
            memoryStream.Position = 0;
            byte[] sheetData = memoryStream.ToArray();
            return File(sheetData, HttpContentMediaTypes.XLSX, "Danh_sach_chuc_danh.xlsx");
        }
        ...
        public async Task<IActionResult> Create(string record = default, string viewMode = default)
        {
            ...
            {
                model = await _DMRepository.GetById(81, record);
            }
            else
            {
                model.MaxStt = await _DMRepository.GetMaxStt(81);
                model.IsActive = 1;
            }
